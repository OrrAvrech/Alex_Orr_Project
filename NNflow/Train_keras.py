...
def log_dir_name(cfg, learning_rate, num_conv_Bulks, kernel_size, activation):

    # The dir-name for the TensorBoard log-dir.
    s = os.path.join(cfg.paths.summaries_current, "lr-{0:.0e}_bulks-{1}_kernel-{2}_act-{3}")
...
def fit_model(cfg, learning_rate, num_conv_Bulks, kernel_size, activation):
    ...
    if cfg.restore.flag:
        model.load_weights(os.path.join(cfg.paths.model_weights))


    log_dir = log_dir_name(cfg, learning_rate, num_conv_Bulks, kernel_size, activation)
    callback_list = [callbacks.TensorBoard(log_dir=log_dir)]
